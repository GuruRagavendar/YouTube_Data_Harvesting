...
with st.sidebar:
    selected = option_menu("Main Menu", ["Home","To Add Channel Details",'Add to SQL',"Frequently Asked Questions"], 
                icons=['house', 'plus','plus', 'question mark'], menu_icon="cast", default_index=0)
...
# To Add Channel Details Menu
    ...
    st.subheader(':red[Please enter the Channel ID to be added in data lake]')
    ...
    if st.button('SCRAP & ADD') and channel_id:
        try:
            ...
            if check_id  is None:
                ...
                data = list(collection.find({},{"Channel":1,'_id':0}))
                df = pd.DataFrame([i['Channel'] for i in data])
...
# Add to SQL tables
elif selected == 'Add to SQL':
    try: 
        st.subheader(':red[Enter the channel id to be added into SQL]') 
        client = pymongo.MongoClient("mongodb://localhost:27017")
        db = client["youtube_dataHarvest"]
        collection = db["channel_details"]  
        channelid = st.text_input('')
        if st.button('ADD to SQL') and channelid:
            mongocdata = collection.find_one({"Channel.channel_id": channelid}, {"Channel": 1, '_id': 0})
            mongovdata = collection.find_one({"Channel.channel_id": channelid}, {"Videos": 1, '_id': 0})
            mongocmdata = collection.find_one({"Channel.channel_id": channelid}, {"comments": 1, '_id': 0})
            cdf = pd.DataFrame([mongocdata['Channel']])
            vdf = pd.DataFrame(mongovdata['Videos'])
            cmdf = pd.DataFrame(mongocmdata['comments'])
            ...
            vdf.to_sql('video_details', con=engine, if_exists='append', index=False)
            cmdf.to_sql('comment_details', con=engine, if_exists='append', index=False)
            print("Inserted into sql with ID:")
          ...
          st.error(f"There is no data with the channel id: {channelid} in the data lake")
